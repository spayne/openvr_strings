...
// 
//   Modified to demonstrate openvr_string
// 
//   This modification demonstrates how you can use the 
//   openvr_string to easily add logging to your open VR application.
//
//   To get started:
//     * build and run this code on your vive 
//     * once you have it working:
//       * search the code below for references to g_print to turn on the logging
//       * inspect your console to see how events, control states etc are updating.
//       * for example, if you just turn on g_print_polled_controller_state then
//         you can watch the controller states update.
//
#include <SDL.h>
#include <GL/glew.h>
...
{
    if (g_print_property_lookup)
    {
        dprintf("\nLooking up property %s for device %d\n", to_string(prop).c_str(), unDevice);
    }
...
bool CMainApplication::BInit()
{
    ...
    dprintf("After Call to init eError is %s\n", to_string(eError).c_str());
...
bool CMainApplication::HandleInput()
{
    ...
    for( vr::TrackedDeviceIndex_t unDevice = 0; unDevice < vr::k_unMaxTrackedDeviceCount; unDevice++ )
    {
        ...
        {
            if (g_print_polled_controller_state)
            {
...
void CMainApplication::ProcessVREvent( const vr::VREvent_t & event )
{
    ...
    if (g_print_all_events)
    {
        dprintf("\nreceived event %d\n%s", event_count++, to_string(event).c_str());
    }
...
void CMainApplication::RenderFrame()
{
    ...
    if ( m_pHMD )
    {
        ...
        if (g_print_render)
        {
            dprintf("Compositor submit left eye texture:\n%s",
                to_string(leftEyeTexture).c_str());
        }
        vr::Texture_t rightEyeTexture = {(void*)rightEyeDesc.m_nResolveTextureId, vr::API_OpenGL, vr::ColorSpace_Gamma };
        vr::VRCompositor()->Submit(vr::Eye_Right, &rightEyeTexture );
        if (g_print_render)
        {
            dprintf("Compositor submit right eye texture:\n%s",
                to_string(rightEyeTexture).c_str());
        }
    }
    ...
    if (g_print_render)
    {
        dprintf("\nDONE RenderingFrame %d\n", frame_count);
    }

    if (g_print_frame_timing)
    {
...
void CMainApplication::DrawControllers()
{
    ...
    for ( vr::TrackedDeviceIndex_t unTrackedDevice = vr::k_unTrackedDeviceIndex_Hmd + 1; unTrackedDevice < vr::k_unMaxTrackedDeviceCount; ++unTrackedDevice )
    {
        ...
        if (g_print_render)
        {
            dprintf("drawing controller %d into vertdataarray\n%s", 
                unTrackedDevice, 
                to_string(m_rTrackedDevicePose[unTrackedDevice]).c_str());
        }
...
void CMainApplication::RenderScene( vr::Hmd_Eye nEye )
{
    if (g_print_render)
    {
        dprintf("RenderScene to Eye %s\n", to_string(nEye).c_str());
    }
    ...
    if( !bIsInputCapturedByAnotherProcess )
    {
        if (g_print_render)
        {
            dprintf(" Render controller axis lines\n");
        }
    ...
    for( uint32_t unTrackedDevice = 0; unTrackedDevice < vr::k_unMaxTrackedDeviceCount; unTrackedDevice++ )
    {
        ...
        if (g_print_render)
        {
            dprintf(" Render controller %d class %s pose:\n%s",		
                        unTrackedDevice,
                        to_string(m_pHMD->GetTrackedDeviceClass(unTrackedDevice)).c_str(),
...
Matrix4 CMainApplication::GetHMDMatrixProjectionEye( vr::Hmd_Eye nEye )
{
    ...
    if (g_print_render)
    {
        dprintf("projection matrix for eye %s:\n%s",
            to_string(nEye).c_str(),
            to_string(mat).c_str());
    }
    ...
    for ( int nDevice = 0; nDevice < vr::k_unMaxTrackedDeviceCount; ++nDevice )
    {
        ...
        if ( m_rTrackedDevicePose[nDevice].bPoseIsValid )
        {
            if (g_print_controller_poses &&
                (g_print_controller_device_index == -1 ||
                g_print_controller_device_index == nDevice))
            {
                dprintf("controller %d pose:\n%s",
                    nDevice, to_string(m_rTrackedDevicePose[nDevice]).c_str());
            }
...
CGLRenderModel *CMainApplication::FindOrLoadRenderModel( const char *pchRenderModelName )
{
    ...
    if( !pRenderModel )
    {
        ...
        if (g_print_rendermodel_load)
        {
            dprintf("loaded render model %s\n%s",
                pchRenderModelName,
                to_string(*pModel).c_str());
        }
